...
5. Оберіть предметну область відповідно до вашого варіанту. В ході роботи надалі розглядатиметься приклад "Каталог бібліотеки".
6. Реалізуйте функції перегляду книг:
   - створіть файли `index.php` для перегляду списку книг
   - створіть файл `book.php` для перегляду однієї книги за її ідентифікатором
   - отримання даних реалізувати за допомогою PDO
7. Реалізуйте реєстрацію нового користувача через форму: 
   - створіть файл `register.php` 
   - у файлі розмістіть форму реєстрації та код обробки цієї форми
   - використовуйте підготовлені запити PDO 
   - переконайтесь, що паролі зберігаються у вигляді хешу.
8. Реалізуйте авторизацію користувача:  
   - створіть файл `login.php`
   - створіть форму входу у файлі `login.php`
   - перевіряйте логін і пароль через PDO у файлі `login.php`
   - реалізуйте збереження сесії для авторизованого користувача.
9. Додайте можливість виходу з авторизованої сесії (logout)  у файлі `logout.php`.
10.  Створіть спільне верхнє меню для всіх сторінок. Для авторизованого користувача розмістіть там привітання та кнопку виходу. Для неавторизованого - кнопку входу
11. Реалізуйте сторінку перегляду списку користувачів (доступна лише для авторизованих) у файлі `users.php`.
12. Додайте захист від SQL-ін'єкцій у всіх запитах (використовуйте підготовлені запити).
13. Перевірте, що всі HTML-сторінки є валідними, використавши [validator.w3.org](https://validator.w3.org/).
14. Для кожного етапу роботи зробіть знімки екрану та додайте їх у звіт з описом кожного скіншота.
15. Додайте програмний код завдання для самостійного виконання.
16. Дайте відповіді на контрольні запитання.
17. Додайте в кінці звіту посилання на репозиторій та папку з лабораторною роботою.
18. Збережіть звіт у форматі PDF.
20. Додайте звіт до репозиторія.

## Приклади

1. [Додавання користувачів](lab-12/src/users1.php)
2. [Перегляд списку користувачів](lab-12/src/users2.php)
3. [Перегляд одного користувача](lab-12/src/users3.php)
4. [Авторизація користувача](lab-12/src/auth1.php)
